...
describe("asset-tokenization", () => {
  ...
  beforeAll(async () => {
    ...
    const airdropTx1 = await provider.connection.requestAirdrop(assetOwner.publicKey, 10 * LAMPORTS_PER_SOL);
    await provider.connection.confirmTransaction(airdropTx1);
    const airdropTx2 = await provider.connection.requestAirdrop(buyer.publicKey, 10 * LAMPORTS_PER_SOL);
  ...
  test("Tokenizes the asset", async () => {
    const assetTokenAccount = await getAssociatedTokenAddress(tokenMint.publicKey, assetPda, true);
    const [metadataAccount] = PublicKey.findProgramAddressSync(
      [Buffer.from("metadata"), TOKEN_METADATA_PROGRAM_ID.toBuffer(), tokenMint.publicKey.toBuffer()],
      TOKEN_METADATA_PROGRAM_ID
    );

    const tx = await program.methods
      .tokenizeAsset("Mumbai Land Token", "MLT", "https://arweave.net/token-metadata")
      .accountsStrict({
        owner: assetOwner.publicKey,
        platformConfig: platformConfigPda,
        asset: assetPda,
  ...
  test("Buys fractions", async () => {
    ...
    const assetTokenAccount = await getAssociatedTokenAddress(tokenMint.publicKey, assetPda, true);
    const buyerTokenAccount = await getAssociatedTokenAddress(tokenMint.publicKey, buyer.publicKey);
