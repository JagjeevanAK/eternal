...
import type {
  IssuerVerificationRequestsResponse as VerificationReviewQueueResponse,
  VerificationRequest,
...
export function AdminReviewQueueScreen() {
  ...
  const [state, setState] = useState<VerificationReviewQueueResponse | null>(null);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const response = await apiFetch<VerificationReviewQueueResponse>(
        ...
        toast.error(error instanceof Error ? error.message : "Failed to load admin review queue.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const refreshQueue = async () => {
    ...
    const refreshedState = await apiFetch<VerificationReviewQueueResponse>(
      "/admin/verification/requests",
      { token },
    );
            ...
            {requests.length === 0 ? (
              <div className="mt-6 rounded-2xl border border-border bg-background p-5">
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  New issuer submissions will appear here when documents are uploaded for review.
                </p>
              </div>
            ) : (
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-4">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-muted-foreground">
                          {request.assetCategory || "Unspecified category"} · {request.owner?.fullName || "Applicant"}
                        </p>
