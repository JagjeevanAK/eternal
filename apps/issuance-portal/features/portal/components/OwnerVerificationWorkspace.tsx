...
import type { VerificationOwnerResponse } from "@/features/portal/types";
...
const logOwnerVerification = (message: string, payload?: unknown) => {
  if (process.env.NODE_ENV === "production") {
    return;
  }

  if (payload === undefined) {
    console.info("[owner-verification]", message);
    return;
  }

  console.info("[owner-verification]", message, payload);
};
...
export function OwnerVerificationWorkspace() {
  ...
  const loadVerificationWorkspace = useCallback(async () => {
    if (!token) {
      logOwnerVerification("Skipping workspace load because there is no session token.");
      setState(null);
      setError(null);
      setLoading(false);
      return;
    ...
    logOwnerVerification("Loading owner verification workspace.", { userId: user?.id ?? null });
    setLoading(true);
    setError(null);

    try {
      const response = await apiFetch<VerificationOwnerResponse>("/verification/requests", { token });
      logOwnerVerification("Workspace loaded.", {
        requestCount: response.requests.length,
        issuerCount: response.issuers.length,
      });
      setState(response);
      setForm((current) => ({
        ...current,
      ...
      console.error("[owner-verification] Workspace load failed.", error);
      setState(null);
      setError(message);
      toast.error(message);
    } finally {
  ...
  const submit = async () => {
    logOwnerVerification("Submit clicked.", {
    ...
    if (!token) {
      logOwnerVerification("Submit aborted because there is no session token.");
      return;
    }

    if (!form.issuerId || !form.assetName.trim()) {
      logOwnerVerification("Submit validation failed: missing issuer or asset name.");
      toast.error("Select an issuer and enter the asset name.");
      return;
    }

    if (form.files.length === 0) {
      logOwnerVerification("Submit validation failed: no files selected.");
      toast.error("Upload at least one document.");
      return;
    }

    if (form.files.length > MAX_FILES) {
      logOwnerVerification("Submit validation failed: too many files.", { fileCount: form.files.length });
    ...
    for (const file of form.files) {
      if (file.size > MAX_FILE_SIZE_BYTES) {
        logOwnerVerification("Submit validation failed: file exceeds size limit.", {
          fileName: file.name,
          size: file.size,
        });
    ...
    try {
      ...
      logOwnerVerification("Sending verification request.", {
        issuerId: form.issuerId,
        assetName: form.assetName.trim(),
        assetCategory: form.assetCategory.trim() || null,
        ownerNoteLength: form.ownerNote.trim().length,
      ...
      logOwnerVerification("Verification request submitted successfully.", {
        requestCount: refreshedState.requests.length,
      });
      ...
      console.error("[owner-verification] Submit failed.", error);
  ...
  const openFile = async (fileId: string) => {
    if (!token) {
      logOwnerVerification("Open file skipped because there is no session token.", { fileId });
      return;
    }

    try {
      logOwnerVerification("Opening verification file.", { fileId });
      await openProtectedFile(`/verification/files/${fileId}`, token);
    } catch (error) {
      console.error("[owner-verification] Failed to open verification file.", error);
    ...
    return (
      <div className="rounded-[2rem] border border-border bg-card p-8">
        ...
        <p className="mt-3 text-sm leading-6 text-muted-foreground">
          The owner verification workspace requires a signed-in investor account.
        </p>
        <Link
  ...
  if (user.role !== "investor") {
    return (
      <div className="rounded-[2rem] border border-border bg-card p-8">
        ...
        <p className="mt-3 text-sm leading-6 text-muted-foreground">
          This workspace is only available to investor accounts.
        </p>
      </div>
    );
  }
